...
function render(){
 ...
 <p>${esc(x.description||"")}</p><div class="bottom"><span>${esc(x.time||"")}</span><a href="${esc(x.link)}" target="_blank" rel="noopener">Read ↗</a></div></div></article>`).join(""):`<div class="empty">No matching stories.</div>`;
}
async function load(){
